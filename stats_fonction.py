...
def statistiques_salaires(personnes , salaires , primes ):
    ...
    personnes_plus_6 = []
    personnes_plus_250 = []
    for i, u in enumerate(salaires):
        if u >= 3000 : 
            personnes_plus_3000.append(personnes[i])
    for i, (salaire, prime) in enumerate(zip(salaires, primes)):
        if (prime * 100) / salaire >= 6:
            personnes_plus_6.append(personnes[i])
    for i, u in enumerate(primes):
        if u >= 250 :        
            personnes_plus_250.append(personnes[i])
    return personnes_plus_3000, personnes_plus_250, personnes_plus_6
        
stat = statistiques_salaires(personnes, salaires, primes)
print(stat)
#corriger
